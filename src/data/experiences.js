const experiences = [
  {
    id: 'matsuo-institute-3',
    company: '株式会社松尾研究所',
    role: 'AIエンジニア（LLMエージェント / RAG研究開発）',
    period: '2026年7月〜現在',
    description:
      '業務Webシステムを自律操作するGUI探索エージェントの研究開発。RAGによる業務知識注入機構の実装と、A/B検証による完遂率・精度・コスト・時間の定量評価を担当。',
    tags: ['Python', 'LLM Agent', 'RAG', 'OpenAI API', 'A/B Testing'],
    url: 'https://matsuo-institute.com/',
    blogUrl: null,
    hasDetail: true,
    details: {
      overview:
        'LLMベースのGUI自律探索エージェント（ブラウザ操作で業務システムの申請フローを自律実行）の研究開発プロジェクトに従事。' +
        '業務文書を検索してエージェントに事前知識を与えるRAG注入機構を設計・実装し、' +
        '業務フロー×RAG有無のA/B検証による定量評価、失敗原因のフォレンジック分析、PM向け報告まで一貫して担当。',
      responsibilities: [
        'RAG知識注入機構の設計・実装: タスク文からLLMが検索クエリを自動生成し、vector store（業務文書群）を検索、要点を「探索ヒント集」に統合してエージェントの指示文へ注入するパイプラインを実装。コードレビュー指摘への対応まで完了',
        '無人A/B検証の設計・実施: 複数業務フロー×RAG有無の比較実験を、実行条件の固定表・実行前ゲートチェック付きの検証プロセスとして設計し、無人バッチで実行',
        '多層の定量評価: 完遂を「プロセス完遂」と「実際の登録成功」の2レイヤーで判定し、登録内容は指示値と実際の入力値の項目単位で照合。RAG有無による完遂率・入力精度・実行時間・コストの差を定量評価',
        '失敗原因のフォレンジック分析: ステップログとスクリーンショットから超過ステップを作業フェーズ別に帰属させて根本原因を特定し、恒久対策の比較資料を作成して意思決定を支援',
        'PM向け検証報告: 結論と検証結果の1対1対応表・テーブル読み順ガイドを備えた報告資料をNotionで整備し、指摘への即日対応を継続',
      ],
      achievements:
        'RAG知識注入により、対象業務フロー全てで完遂率・入力精度・実行時間・コストが改善することを定量実証。' +
        '導入判断に必要な根拠データと恒久対策の意思決定資料を提供',
      techStack: [
        'Python',
        'OpenAI API',
        'LLM Agents',
