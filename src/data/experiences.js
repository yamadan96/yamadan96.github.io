const experiences = [
  ...
  },
  {
    ...
    details: {
      ...
      achievements:
        '鉄パイプ計数で束単位精度100%・推論0.2秒/枚達成（テックリード）、音声合成がトヨタイムズ・PR TIMESに掲載',
      techStack: [
        'Python',
        'PyTorch',
        'YOLOv8',
  ...
  },
  {
    ...
    details: {
      overview:
        ...
        '自律エージェントとワークフロー型制御を組み合わせ、カメラ映像から環境を認識して「見て→判断して→動く」プロセスを実現。' +
        'エッジデバイス上でマルチモーダルLLM（VLM）を動作させ、インターネット接続不要の100%ローカル実行が可能なデモシステムを開発した。',
      responsibilities: [
        '【ハードウェア×VLMの統合設計】Semantic Kernelのプラグインアーキテクチャを採用し、深度カメラ（Intel RealSense）・マイコン制御（Arduino）・VLM推論を疎結合に統合。各モジュールを差し替え可能な構造とした',
        '【危険物・異常検知タスクの設計】刃物・転倒リスク物・通路障害物などの検知対象を定義し、検出→優先度判断→対応提案の3段階推論パイプラインを構築して、判断根拠を説明できるシステムを実現',
        '【エッジ推論環境の構築】クラウドに依存せず動作するマルチモーダルLLM推論環境をローカルに構築し、リアルタイムでの環境認識・解析・制御が可能なプロトタイプを開発・検証',
        '【今後の方向性の提案】ローカルモデル化・分散処理・ロボット統合を軸とした発展方針を整理し、プロジェクトの次フェーズに向けた提案としてまとめた',
      ],
      achievements:
        'プロトタイプAIエージェントを開発し、見落としの削減と判断説明の一貫性向上を確認。100%ローカル実行のデモを完成させ、フィジカルAI領域への発展方針を提案',
      techStack: [
        'Python',
