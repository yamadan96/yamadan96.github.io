const profile = {
  name: '山田 悠人',
  nameEn: 'Yuto Yamada',
  title: 'AI Engineer',
  roles: ['AI Engineer', 'ML Researcher', 'Full-Stack Developer'],
  affiliation: '東京理科大学大学院 創域理工学研究科 電気電子情報工学専攻 M1',
  career: '2027年4月〜 外資系IT企業 AIアーキテクト職 入社予定',
  bio: [
    'AI・機械学習を専門とするエンジニア。',
    '松尾研究所やMIXI、LegalOn Technologies等、16社以上での実務・インターン経験を持つ。',
    '研究テーマは災害時の建物被害をAIで判定する技術。学会発表3件。研究だけでなく、実際に使えるシステムとして形にするところまで一貫して手がける。',
    '2027年4月より外資系IT企業にAIアーキテクトとして入社予定。',
  ],
  stats: [
    { label: 'インターン経験', value: '16社+' },
    { label: '学会発表', value: '3件' },
    { label: '長期インターン', value: '3社' },
  ],
  social: [
    { platform: 'github', url: 'https://github.com/yamadan96' },
    { platform: 'qiita', url: 'https://qiita.com/yamadan96' },
    { platform: 'x', url: 'https://x.com/dev2rich' },
    { platform: 'linkedin', url: 'https://www.linkedin.com/in/悠人-山田-156149304' },
    { platform: 'note', url: 'https://note.com/yuto_yamada398' },
    { platform: 'zenn', url: 'https://zenn.dev/yuto0' },
    { platform: 'wantedly', url: 'https://www.wantedly.com/id/yutoyamada0' },
  ],
  email: 'yuto.yamada0101@gmail.com',
  achievements: [
    {
      title: 'インターン経験16社+',
      icon: '🏢',
      description: '松尾研究所、LegalOn Technologies、MIXI、エクサウィザーズ、GMOメディア、ログラス等',
    },
    {
      title: '学会発表 3件',
      icon: '📄',
      description: 'FIT2025（口頭）、IEICE2026総合大会（口頭）、映像情報メディア学会2026年年次大会（口頭・予定）',
      link: 'https://www.ieice.org/publications/conferences/summary.php?id=FIT0000017580&expandable=2&ConfCd=F&session_num=7n&lecture_number=I-029&year=2025&conf_type=F',
    },
    {
      title: 'DECC 2025 ファイナリスト',
      icon: '🏆',
      description: 'DISCOプログラミングコンテスト本戦出場',
      link: 'https://news.mynavi.jp/techplus/kikaku/disco_decc-2/',
    },
    {
      title: '松尾研 LLMATCHプログラム',
      icon: '🎓',
      description: '東京大学 松尾・岩澤研究室 選抜制LLMプログラム参加',
    },
    {
      title: 'トヨタイムズ掲載',
      icon: '📺',
      description: 'Airionでの音声合成AI開発の成果がトヨタイムズニュースに掲載',
      link: 'https://prtimes.jp/main/html/rd/p/000000004.000118893.html',
    },
    {
      title: 'ギネス世界記録イベント参加',
      icon: '🌍',
      description: '8時間リレーにおける50m走最多人数（2025年12月、千葉県野田市）',
    },
  ],
};

export default profile;
